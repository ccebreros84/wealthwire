...
// SheetJS reads raw CSV bytes as Latin-1, which turns UTF-8 "Nestlé" into
// "NestlÃ©". Decode the text ourselves: UTF-8 first, and if the bytes are not
// valid UTF-8, Windows-1252 (what Excel writes for "CSV" on most machines).
...
export async function parseFile(file) {
  ...
  const wb = isCsv(file)
    ? XLSX.read(decodeCsv(buf), { type: 'string', cellDates: false })
    : XLSX.read(buf, { cellDates: false });
  const sheetName = wb.SheetNames.find(n => normalise(n) === 'orders') || wb.SheetNames[0];
  const ws = wb.Sheets[sheetName];
  if (!ws) return { headerError: { code: 'fileNoSheet', message: 'That file has no readable sheet.' }, rows: [] };

  const grid = XLSX.utils.sheet_to_json(ws, { header: 1, blankrows: false, raw: false, defval: '' });
  if (!grid.length) return { headerError: { code: 'fileEmpty', message: 'That sheet is empty.' }, rows: [] };
  ...
  if (missing.length) {
    return {
      headerError: {
        code: 'fileHeaderMissing',
        params: { missing: missing.join(', ') },
        message: 'The header row does not match the template. Missing: ' + missing.join(', ') + '.',
      },
      rows: [],
    };
  }
  ...
  return {
    headerError: rows.length ? null : { code: 'fileNoRows', message: 'The sheet has a valid header but no order rows.' },
    rows,
  };
}
...
export function validate(rows) {
  const issues = {};   // rowId -> { field: { level, message, code, params } }
  const seen = new Map();
  let errors = 0, warnings = 0;

  // `code` + `params` let the UI localise the message; `message` is the English fallback.
  const flag = (row, field, level, message, code, params) => {
    issues[row.id] = issues[row.id] || {};
    if (issues[row.id][field] && issues[row.id][field].level === 'error') return;
    issues[row.id][field] = { level, message, code, params };
    if (level === 'error') errors++; else warnings++;
  };

  rows.forEach(row => {
    COLUMNS.filter(c => c.required).forEach(c => {
      if (!row[c.key]) flag(row, c.key, 'error', c.label + ' is required', 'required', { field: c.label });
    });

    if (row.isin) {
      if (row.isin.length !== 12) flag(row, 'isin', 'error', 'ISIN must be 12 characters, this one has ' + row.isin.length, 'isinLength', { n: row.isin.length });
      else if (!isinValid(row.isin)) flag(row, 'isin', 'error', 'ISIN check digit does not match', 'isinCheck');
    }
    if (row.side && row.side !== 'BUY' && row.side !== 'SELL') {
      flag(row, 'side', 'error', 'Side must be BUY or SELL', 'side');
    }
    if (row.quantity) {
      const q = Number(String(row.quantity).replace(/[' ]/g, '').replace(',', '.'));
      if (!Number.isFinite(q) || q <= 0) flag(row, 'quantity', 'error', 'Quantity must be a positive number', 'quantity');
    }
    if (row.ccy && !/^[A-Z]{3}$/.test(row.ccy)) {
      flag(row, 'ccy', 'error', 'Currency must be a 3-letter code', 'ccy');
    }
    if (row.orderType === 'LIMIT' && !row.limit) {
      flag(row, 'limit', 'warning', 'LIMIT order without a limit price', 'limitMissing');
    }
    ...
    if (cls.declared) {
      if (!ELIGIBLE.includes(cls.klass)) {
        flag(row, 'assetClass', 'error',
          ...
          'notEligibleDeclared', { klass: cls.klass });
      } else {
        ...
        if (!ELIGIBLE.includes(inferred.klass)) {
          flag(row, 'instrument', 'warning',
            ...
            'declaredMismatch', { klass: cls.klass, klassKey: inferred.klass });
        }
      }
      ...
      flag(row, 'instrument', 'error',
        ...
        'looksIneligible', { klassKey: cls.klass });
    }

    const key = [row.isin, row.side, row.account].join('|');
    if (row.isin && seen.has(key)) {
      flag(row, 'instrument', 'warning', 'Same instrument, side and account as row ' + seen.get(key), 'duplicate', { row: seen.get(key) });
    } else if (row.isin) {
...
// 1234567.8 -> "1,234,568". Comma grouping matches the blotter on the landing
// page. Swiss convention is an apostrophe (1'234'568) - switch both together if
// you want the local format.
...
export function downloadExecutions(rows, execs, meta = {}) {
  const wb = XLSX.utils.book_new();
  const bank = meta.bank || '';

  // Original sheet columns, then the execution fields shown in the blotter,
  // using the same vocabulary: Filled / Rejected, avg price, bank, client ref.
  const header = [
    ...HEADERS,
    'Status', 'Filled qty', 'Avg price (simulated)', 'Notional',
    'Bank', 'Routed at', 'Completed at', 'Execution ID', 'Reject reason',
  ];

  const STATUS_LABEL = { filled: 'Filled', rejected: 'Rejected', pending: 'Working' };
  ...
  ws['!cols'] = [
    // Instrument, ISIN, Asset class, Side, Quantity, Order type, Limit, CCY, Account, Client ref
    { wch: 26 }, { wch: 16 }, { wch: 12 }, { wch: 8 }, { wch: 10 },
    { wch: 12 }, { wch: 10 }, { wch: 7 }, { wch: 18 }, { wch: 16 },
    // Status, Filled qty, Avg price, Notional, Bank, Routed at, Completed at, Exec ID, Reason
    { wch: 12 }, { wch: 11 }, { wch: 20 }, { wch: 14 }, { wch: 14 },
    { wch: 11 }, { wch: 20 }, { wch: 16 }, { wch: 36 },
  ];
